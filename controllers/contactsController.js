...
const getAllContactsController = async (req, res) => {
  ...
  const allContacts = await contacts.listContacts(ownerId, page, limit, favorite);
...
const addContactController = async (req, res) => {
  ...
  const contact = await contacts.addContact(name, email, phone, favorite, ownerId);
...
const updateStatusContactController = async (req, res) => {
  const {_id: ownerId} = req.user;
  const id = req.params.contactId;
  const updatedContact = await contacts.updateStatusContact(id, req.body, ownerId);
  res.json(updatedContact);
};
